...
void CAN_IRQ_RX_Pending_disable(FDCAN_HandleTypeDef *can, uint32_t fifo) {
  CLEAR_BIT(can->Instance->IE,
            fifo == FDCAN_RX_FIFO0 ? FDCAN_IT_RX_FIFO0_NEW_MESSAGE : FDCAN_IT_RX_FIFO1_NEW_MESSAGE);
}

void CAN_IRQ_TX_Complete_enable(FDCAN_HandleTypeDef *can) {
  SET_BIT(can->Instance->IE, FDCAN_IE_TCE);
  WRITE_REG(can->Instance->TXBTIE, FDCAN_TXBTIE_TIE);
}

void CAN_IRQ_TX_Complete_disable(FDCAN_HandleTypeDef *can) {
...
int CAN_send_packet(can_msg *msg) {
  ...
  header.DataLength = msg->size << 16u;
...
int CAN_poll_TX() {
  if (HAL_FDCAN_GetTxFifoFreeLevel(&hcan) > 0) { // Free buffers in hardware, send
    if (!messages_tx.is_empty()) {
      ...
      if (HAL_FDCAN_AddMessageToTxFifoQ(&hcan, &header, msg.data.u8) != HAL_OK) {
...
void MX_FDCAN1_Init() {
  ...
  if ((res = HAL_FDCAN_ConfigGlobalFilter(&hcan, FDCAN_REJECT, FDCAN_REJECT, FDCAN_REJECT_REMOTE,
  ...
  if (__HAL_FDCAN_GET_FLAG(&hcan, FDCAN_IT_RX_FIFO0_NEW_MESSAGE)) {
    ...
    if (HAL_FDCAN_GetRxFifoFillLevel(&hcan, FDCAN_RX_FIFO0) > 0) {
      ...
      if (!messages_rx.is_full()) {
        ...
        if ((res = HAL_FDCAN_GetRxMessage(&hcan, FDCAN_RX_FIFO0, &rx_header, msg.data.u8))
            == HAL_OK) {
