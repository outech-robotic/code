  ...
  while (true) {
    ...
    if (canpb.receive_msg(msg_rx)) {
      switch (msg_rx.which_message_content) {
        case BusMessage_stopMoving_tag:serial.println("Packet Received: StopMoving");
          mcs.stop(false);
          break;

        case BusMessage_translate_tag:serial.println("Packet Received: Translate");
          mcs.translate_ticks(msg_rx.message_content.translate.ticks);
          break;

        case BusMessage_rotate_tag:serial.println("Packet Received: Rotate");
        ...
        case BusMessage_moveWheelAtSpeed_tag:serial.println("Packet Received: MoveWheelAtSpeed");
          mcs.set_target_speed(Motor::Side::LEFT,
                               msg_rx.message_content.moveWheelAtSpeed.left_tick_per_sec);
          mcs.set_target_speed(Motor::Side::RIGHT,
        ...
        case BusMessage_setMotionControlMode_tag:serial.println("Packet Received: SetMotionControlMode");
          mcs.set_control(msg_rx.message_content.setMotionControlMode.speed,
                          msg_rx.message_content.setMotionControlMode.translation,
                          msg_rx.message_content.setMotionControlMode.rotation);
          break;
    ...
    if (position_timer.check()) {
      ...
      if (mcs_stop_status != MotionController::STOP_STATUS::NONE) {
        msg_move_end.message_content.movementEnded.blocked = (mcs_stop_status ==
            MotionController::STOP_STATUS::BLOCKED);
