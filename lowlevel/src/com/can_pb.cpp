...
BufferPool<16, LogMessage> log_pool;
...
bool encode(pb_ostream_t *stream, const pb_field_t *field, void * const *arg) {
   ...
   if(stream && field->tag == DebugLog_content_tag) {
      ...
      bool encode_status = pb_encode_string(stream, (uint8_t *)*arg, strlen((char*)(*arg)));
   }
...
int Can_PB::update() {
   ...
   if (is_tx_available()) {
      ...
      if (!pb_msg_tx_buffer.is_empty()) {
         ...
         msg.message_content.debugLog.content.funcs.encode = &encode;
