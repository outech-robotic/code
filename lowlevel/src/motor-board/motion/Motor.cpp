...
Motor::Motor(const Side new_side) : side(new_side),
                                    dir((new_side == Side::LEFT) ? (Direction::FORWARD) : (Direction::BACKWARD)) {}
...
void Motor::set_direction(Direction new_dir) {
  ...
  if (side == Side::LEFT) {
    switch (dir) {
      case Direction::FORWARD:digitalWrite(PIN_DIR_L1, GPIO_HIGH);
        digitalWrite(PIN_DIR_L2, GPIO_LOW);
        break;
      case Direction::BACKWARD:digitalWrite(PIN_DIR_L2, GPIO_HIGH);
        digitalWrite(PIN_DIR_L1, GPIO_LOW);
        break;
      case Direction::BRAKE:digitalWrite(PIN_DIR_L1, GPIO_HIGH);
    ...
    switch (dir) {
      case Direction::FORWARD: //For other side, pins are inverted
        ...
        break;
      case Direction::BACKWARD:digitalWrite(PIN_DIR_R2, GPIO_HIGH);
        digitalWrite(PIN_DIR_R1, GPIO_LOW);
        break;
      case Direction::BRAKE:digitalWrite(PIN_DIR_R1, GPIO_HIGH);
