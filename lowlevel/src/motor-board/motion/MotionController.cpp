...
void MotionController::update_position() {
  ...
  robot_status.rotation_total =
      ((cod_right.current - robot_status.translation_total) - (cod_left.current - robot_status.translation_total))
          >> 1;
...
void MotionController::control_motion() {
  ...
  if (robot_status.controlled_position || robot_status.controlled_rotation || robot_status.controlled_speed) {
    ...
    if (robot_status.controlled_position || robot_status.controlled_rotation) {
      speed_sp_translation = pid_translation.compute(robot_status.translation_total,
                                                     robot_status.translation_setpoint);
      ...
      cod_left.speed_setpoint = ((int32_t) speed_sp_translation -
          (int32_t) speed_sp_rotation); // Clockwise rotation is positive
...
void MotionController::set_target_speed(Motor::Side side, int32_t speed) {
  switch (side) {
    case Motor::Side::LEFT:cod_left.speed_setpoint_wanted = speed;
      break;
    case Motor::Side::RIGHT:cod_right.speed_setpoint_wanted = speed;
      break;
  }
...
void MotionController::detect_stop() {
  ...
  // If not moving
  if ((robot_status.moving && !robot_status.forced_movement) && (pid_translation.get_derivative() == 0) &&
      (pid_rotation.get_derivative() == 0)) {
    if (time_stop > 0) {
      time_stop--;
    } else {
      // Close enough to setpoints
      if ((err_trans <= robot_status.translation_tolerance)
          && (err_rot <= robot_status.rotation_tolerance)) {
        stop(false);
      }
        // Probably blocked
      else {
...
int32_t MotionController::get_data_from_pid(PID_FP &pid, uint8_t requested_data) {
  ...
  switch (requested_data) {
    ...
    case PID_DATA_ID::DERIVATIVE:ret_data = pid.get_derivative();
      break;
    case PID_DATA_ID::ERROR:ret_data = pid.get_error();
      break;
    default:ret_data = -1;
      break;
  }
...
int32_t MotionController::get_pid_data(uint8_t id, uint8_t requested_data) {
  ...
  switch (id) {
    ...
    case PID_ID::PID_RIGHT_SPEED:ret_val = get_data_from_pid(pid_speed_right, requested_data);
      break;
    case PID_ID::PID_TRANSLATION:ret_val = get_data_from_pid(pid_translation, requested_data);
      break;
    case PID_ID::PID_ROTATION:ret_val = get_data_from_pid(pid_rotation, requested_data);
      break;
    default:ret_val = -1;
      break;
  }
...
void MotionController::set_raw_pwm(Motor::Side side, int16_t pwm) {
  switch (side) {
    case Motor::Side::LEFT :motor_left.set_pwm(pwm);
      break;
    case Motor::Side::RIGHT:motor_right.set_pwm(pwm);
      break;
  }
}
...
void MotionController::set_kp(uint8_t id, uint32_t k) {
  switch (id) {
    ...
    case PID_ID::PID_RIGHT_SPEED :pid_speed_right.set_kp(k);
      break;
    case PID_ID::PID_TRANSLATION :pid_translation.set_kp(k);
      break;
    case PID_ID::PID_ROTATION :pid_rotation.set_kp(k);
      break;
    default:while (true);
  }
}
...
void MotionController::set_ki(uint8_t id, uint32_t k) {
  switch (id) {
    ...
    case PID_ID::PID_RIGHT_SPEED :pid_speed_right.set_ki(k, MOTION_CONTROL_FREQ);
      break;
    case PID_ID::PID_TRANSLATION :pid_translation.set_ki(k, MOTION_CONTROL_FREQ);
      break;
    case PID_ID::PID_ROTATION :pid_rotation.set_ki(k, MOTION_CONTROL_FREQ);
      break;
    default:while (true);
  }
}
...
void MotionController::set_kd(uint8_t id, uint32_t k) {
  switch (id) {
    ...
    case PID_ID::PID_RIGHT_SPEED :pid_speed_right.set_kd(k, MOTION_CONTROL_FREQ);
      break;
    case PID_ID::PID_TRANSLATION :pid_translation.set_kd(k, MOTION_CONTROL_FREQ);
      break;
    case PID_ID::PID_ROTATION :pid_rotation.set_kd(k, MOTION_CONTROL_FREQ);
      break;
    default:while (true);
  }
}
...
void MotionController::set_limits(uint16_t speed_translation, uint16_t speed_rotation, uint16_t speed_wheel,
                                  uint16_t accel_wheel) {
