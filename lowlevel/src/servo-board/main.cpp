    ...
    while (1) {
            ...
            if (serial.read(str, 10)) {
                ...
                if (!servo_selected) {
                    ...
                    if (data > 180) {
                        ...
                        switch (servo_id) {
                            case 1 :
                                PWM_write_angle(PIN_PWM_1, data);
                                break;
                            case 2 :
                                PWM_write_angle(PIN_PWM_2, data);
                                break;
                            case 3 :
                                PWM_write_angle(PIN_PWM_3, data);
                                break;
                        }
                    }
        ...
        if ((CAN_receive_packet(&rx_msg)) == HAL_OK) {
            if (CAN_PKT_MESSAG_ID(rx_msg.header.StdId) == (CAN_MSG_SERVO_POS << CAN_BOARD_ID_WIDTH | CAN_BOARD_ID)) {
                ...
                switch (id) {
                    case 1 :
                        PWM_write_angle(PIN_PWM_1, value);
                        break;
                    case 2 :
                        PWM_write_angle(PIN_PWM_2, value);
                        break;
                    case 3 :
                        PWM_write_angle(PIN_PWM_3, value);
                        break;
                    default:
                        serial.printf("WRONG SERVO ID\r\n");
                        break;
                }
            }
        }
