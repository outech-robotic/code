...
class ActuatorGateway:
    ...
    async def _send_message(self, adapter_id: int,
                            message: BusMessage) -> None:
        """
        Serializes and sends sends a protobuf BusMessage through an adapter.
        """
        if adapter_id >= len(self._servo_board_adapters):
            raise RuntimeError(
                f"Board ID out of bounds:"
                f"{adapter_id}>={len(self._servo_board_adapters)}")
        payload = message.SerializeToString()
        await self._servo_board_adapters[adapter_id].send(payload)

    async def move_servo(self, board_id: int, servo_id: int,
                         angle: int) -> None:
        """
